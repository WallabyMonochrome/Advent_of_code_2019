...
function computeWire(wire) {
    ...
    wire.forEach(position => {
        const shift = parseInt(position.substring(1));
        const direction = position[0];
        step += shift;
        const positionFormat = { x, y, shift, direction, step };
        switch (direction) {
            case 'R':
                positionFormat.shiftX = x + shift;
                x += shift;
                break;
            case 'L':
                positionFormat.shiftX = x - shift;
            ...
            case 'U':
                ...
                break;
            default:
                console.log("Invalid direction: ", position[0]);
                break;
        }
        powerGrid.push(positionFormat);
    });
    return powerGrid;
}

function checkManhattan(value1, value2) {
    return Math.abs(value2.x - value1.x) + Math.abs(value2.y - value1.y);
}

function checkCross(positionA, positionB) {
    let horizontal;
    let vertical;

    if (positionA.direction === 'L' || positionA.direction === 'R') {
    ...
    if ((vertical.y <= horizontal.y && vertical.shiftY >= horizontal.y) ||
        (vertical.y >= horizontal.y && vertical.shiftY <= horizontal.y)) {
        if ((horizontal.x <= vertical.x && horizontal.shiftX >= vertical.x) ||
            (horizontal.x >= vertical.x && horizontal.shiftX <= vertical.x)
        ) {
            const collision = { x: vertical.x, y: horizontal.y };
            if (collision.x === 0 && collision.y === 0) {
                return false;
            }
            const stepsToReach = horizontal.step - Math.abs(horizontal.shiftX - vertical.x) + vertical.step - Math.abs(vertical.shiftY - horizontal.y);
            // console.log("Step:", horizontal.step - Math.abs(horizontal.shiftX - vertical.x), vertical.step - Math.abs(vertical.shiftY - horizontal.y));
            collision.step = stepsToReach;
            return collision;
        }
    }
    return false;
...
function checkForCollision(wireA, wireB) {

    // Dictionnary to exclude same axis collision
    const dicoColission = [];
    dicoColission['R'] = 'L';
    dicoColission['L'] = 'R';
    dicoColission['U'] = 'D';
    ...
    const collisionTab = [];
    wireA.forEach(positionA => {
        const directionA = positionA.direction;
        wireB.forEach(positionB => {
            const directionB = positionB.direction;
            if (directionA != directionB && dicoColission[directionA] != directionB) {
                const didCross = checkCross(positionA, positionB);
                if (didCross) {
                    collisionTab.push(didCross);
                };
            }
        });
    });
    let resultArray = [];
    // -------- Part 1 with Manhattan ---------
    // collisionTab.forEach(collision => {
    //      resultArray.push(checkManhattan({ x: 0, y: 0 }, collision));
    // });
    // -----------------------------------------

    // ------------Part 2 with minimum step ---------- //
    resultArray = collisionTab.map(coll => (coll.step));
    // --------------------------------------------------
    console.log("Resultat: ", Math.min(...resultArray));
}

const gridA = computeWire(wire1);
const gridB = computeWire(wire2);
checkForCollision(gridA, gridB);